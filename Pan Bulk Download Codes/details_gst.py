...
def gstinDownload(pan):
	...
	try:
		...
		def read_captcha2(captcha_path):
			...
			# ocr_text = 'tesseract '+'"'+bmp_path+'" '+'"'+txt_path+'.txt"'+' "--psm" '+'"/DriveU/GSTCode/letters_small"'
			# ocr_text = 'tesseract '+'"'+bmp_path+'" '+'"'+txt_path+'" '+"--psm 8"+'"/DriveU/GSTCode/letters_small"'
		...
		# maindir_save = "/DriveD/GST/details/"
		# if not os.path.exists(maindir_save):
		# 	os.makedirs(maindir_save)

		input_dir = "/DriveD/GST/main_page/"+pan+"/"

		listfiles = os.listdir(input_dir)
		listfiles = [input_dir+x for x in listfiles]
