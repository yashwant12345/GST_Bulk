...
gst_log = "/DriveD/GST/log/"
if not os.path.exists(gst_log):
	os.makedirs(gst_log)
...
def sendmail(msg_body):
	...
	# storing the subject  
	msg['Subject'] = "GST Bulk."
	  
	# string to store the body of the mail 
	body = str(msg_body) + " cin downloaded in an hour."
...
try:
	with open(gst_log+DT+'_hourly_start_end_log.txt','a') as apd:
		apd.write("Start time "+str(datetime.now())+'\n')
	with open("/DriveD/GST/Codes/count_file.txt",'r') as rd:
	...
	df = df.append({"Process Name":"GST Bulk Download","Folder Name":"","Send Count":str(hour_count),'Error Count':"",'Invalid Error Count':"",'Total Count':str(total_count),"Start Time":"","End Time":"","TimeDelta":""},ignore_index=True)
	...
	with open("/DriveD/GST/Codes/count_file.txt",'w') as wr:
