...
def MainFunctionCall(inputrow):
	...
	def read_captcha(img_path):
		...
		# img_path = r"/DriveD/GST/sample_captcha/4df08adf-05dc-4424-8ac8-c3f893b1c013.png"
...
# file = "/DriveD/GST/input/pan_ic_list.txt"

folder = "/DriveD/input/inputs2/"
listfiles = os.listdir(folder)
listfiles = [folder+x for x in listfiles if "_Pending.txt" in x]
if len(listfiles) == 0:
	# print ("here")
	listfiles = os.listdir(folder)
	listfiles = [folder+x for x in listfiles if "_New.txt" in x]

file = listfiles[0]

# file="/DriveD/GST/input/for GSt.txt"
with open(file,'r') as rd:
	file_data = rd.readlines()

pending_file = file.replace("_New.txt","_Pending.txt")
...
file_data = ["_".join(x.split("|")[::-1]) for x in file_data]
# print (file_data[:2])

check_folder1 = "/DriveD/main_page"
check_list1 = os.listdir(check_folder1)
check_list1 = ["_".join(x.split("_")[:2]) for x in check_list1]
# print (check_list1[:2])

check_folder2 = "/DriveD/main_page_2"
check_list2 = os.listdir(check_folder2)
check_list2 = ["_".join(x.split("_")[:2]) for x in check_list2]
# print (check_list2[:5])
pool_data = list(set(file_data)-set(check_list1))
# print (pool_data[:2])
# exit()
if len(check_list2)!=0:
	# print ("here")
	# print (len(check_list2))
	final_list = list(set(pool_data)-set(check_list2))
	# print (len(final_list))
else:
	final_list = pool_data
...
if len(final_list) == 0:
	...
	results = [pool.apply_async(MainFunctionCall, args=(value,)) for value in final_list]
	pool.close()
	pool.join()
	flag4 = 1
	while flag4 == 1:
