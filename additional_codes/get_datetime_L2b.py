...
# if os.path.exists("/DriveD/additional_codes/file_with_datetime_L2b.txt"):
# 	os.remove("/DriveD/additional_codes/file_with_datetime_L2b.txt")

# if os.path.exists("/DriveD/additional_codes/file_with_date_only_L2b.txt"):
# 	os.remove("/DriveD/additional_codes/file_with_date_only_L2b.txt")

with open("/DriveD/additional_codes/file_with_date_only_L2b.txt",'r') as rd:
	...
	if len(listfiles) !=0:
		...
		try:
			...
			for file in listfiles:
				...
				with open("/DriveD/additional_codes/file_with_datetime_L2b.txt",'a') as ap:
					ap.write(file+"****"+str(folderdate)+"\n")

				with open("/DriveD/additional_codes/file_with_date_only_L2b.txt",'a') as ap:
...
for distinct_date in distinct_date_data:
	...
	dict_ = {"ServerName":"GST L2b","date":distinct_date,"GST L2b Count":str(len(df_date))}

	masterdf = masterdf.append(dict_,ignore_index=True)

total_count = sum(count_list)

masterdf["GST L2b Total Count"] = str(total_count)
masterdf = masterdf[masterdf["date"]==p_date]
masterdf = masterdf[["ServerName","date","GST L2b Count","GST L2b Total Count"]]

csv_name = "/DriveD/additional_codes/output/gst_L2b_"+str(c_date)+"_New.csv"
